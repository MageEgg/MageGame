...
//SpellHeroSkillAction(unit wu,unit tu,real sx,real sy,int id,int lv,int index)
   ...
   function GetUnitAttackNumb(unit wu)->int //S036
    ...
    function SpellS028(unit u0,unit u10,real damage1)//冥月流星落
        ...
        if  GetUnitAbilityLevel(wu,'S028') == 0
            UnitTimerAddSkill(wu,'S028',3)
            AddUnitStateExTimer(wu,71,6,3)
        endif
   ...
   function SpellS052(unit u1,unit tu,real damage1)//造成伤害减少其他技能0.5CD
                ...
                GroupEnumUnitsInRange(g.ejg,x1,y1,100,GroupHasUnitAddBuff(GetOwningPlayer(u),g1,"",'DB02',1,852095))  
                   loop
                        ...
                        if GetUnitAbilityLevel(gu, 'B003')>0
                            UnitDamageTarget(u,gu,damage*1.3,false,false,ATTACK_TYPE_CHAOS,DAMAGE_TYPE_MAGIC,null)
                        else
    ...
    function SpellS075(unit u,unit u1)//夺命刺
        group gg = CreateGroup()
        int id = 'S075'
        real damage = GetAbilityDamage(wu,id,GetHeroAbilityLevel(wu,id))
        real x = 0
    ...
    function SpellS101(unit u,real x,real y,real damage)
        ...
        UnitDamageGroup(u,g.ejg,damage+(GetUnitRealState(u,5)*4),false,false,ATTACK_TYPE_CHAOS,DAMAGE_TYPE_MAGIC,null)
        g.destroy()
        u = null
        flush locals
    endfunction 
